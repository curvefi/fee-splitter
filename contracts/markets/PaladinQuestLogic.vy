...
def __init__(
    crvusd: address,
    quest_board: address,
    incentives_manager: address,
    min_amount_per_vote: uint256,
    max_amount_per_vote: uint256,
):
    """
    ...
    assert (
        min_amount_per_vote < max_amount_per_vote
    ), "invalid: min_amount_per_vote > max_amount_per_vote"
...
def bribe(gauge: address, amount: uint256, data: Bytes[1024]):
    ...
    quest_id = self.create_quest(
        gauge, total_budget, self.min_reward_per_vote, self.max_reward_per_vote
    )
...
def create_quest(
    ...
    fee_ratio: uint256 = staticcall self.quest_board.customPlatformFeeRatio(
        self
    )
    if fee_ratio == 0:
...
def withdraw_expired_quest(quest_id: uint256) -> uint256:
    withdrawable: uint256 = staticcall self.quest_board.questWithdrawableAmount(
        quest_id
    )
    if withdrawable == 0:
        return 0
