...
def test_create_bounty(stakedao_logic, stakedao_market, crvusd):
    stakedao_logic.internal.create_bounty(
        gauge := boa.env.generate_address(), 400, 1234
    )
    ...
    assert stakedao_market.creation_upgradeable()


def test_increase_bounty_duration(stakedao_logic, stakedao_market):
    random_id = 43958
    stakedao_logic.internal.increase_bounty_duration(random_id, 400, 1234)
...
def test_bribe(stakedao_logic, stakedao_market, crvusd, manager):
    ...
    with boa.env.prank(manager.address):
        stakedao_logic.bribe(
            random_gauge, 400, bytes(encoded_max_amount_per_vote)
        )
    ...
    increase_amount = 401
    encoded_increase_max_amount_per_vote = encode(
        ["uint256"], [increase_max_amount_per_vote]
    )
    crvusd.mint_for_testing(stakedao_logic, leftover_crvusd)
    ...
    assert (
        stakedao_market.increase_newMaxPricePerVote()
