...
def test_bribe_subsequent(quest_logic, quest_market, crvusd, manager):
    ...
    # we test for a second call because to withdraw from the previous quest and rollover the budget
...
def test_withdraw_from_quest(quest_logic, quest_market, token_rescuer, crvusd):
    ...
    with boa.env.prank(token_rescuer):
        quest_logic.withdraw_from_quest(7890, recovery_addr := boa.env.generate_address())
    assert crvusd.balanceOf(recovery_addr) == 1000
    assert quest_market.withdraw_id() == 7890

def test_withdraw_from_quest_no_funds(quest_logic, quest_market, token_rescuer):
    with boa.env.prank(token_rescuer):
...
def test_set_voter_blacklist(quest_logic):
    new_list = [boa.env.generate_address(), boa.env.generate_address(), boa.env.generate_address()]
    with boa.reverts("access_control: account is missing role"):
        quest_logic.set_voter_blacklist(new_list)
