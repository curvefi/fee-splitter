...
def test_expected_behavior(manager, bribe_proposer, bribe_manager):
    # Round 1
    payloads = [
        (boa.env.generate_address(), i * 10**19, bytes())
        for i in range(1, 4)
    ]
    ...
    assert manager.total_incentives() == sum(
        [amount for _, amount, _ in payloads]
    )
    ...
    # Round 2 (overwrite)
    payloads2 = [
        (boa.env.generate_address(), i * 10**18, bytes())
        for i in range(1, 4)
    ]
    ...
    assert manager.total_incentives() == sum(
        [amount for _, amount, _ in payloads2]
    )
...
def test_more_than_cap(manager, bribe_proposer):
    reverting_payload = [
        ...
        manager.MAX_INCENTIVES_PER_GAUGE() + 1,
        bytes(),
    ]
