...
def test_expected_behavior(manager, crvusd, bribe_manager, bribe_proposer,
...
def test_zero_address(crvusd, bribe_manager, bribe_proposer, token_rescuer, emergency_admin):
    zero = boa.eval("empty(address)")
    with boa.reverts("zeroaddr: managed_asset"):
        IncentivesManager(zero, bribe_manager, bribe_proposer, token_rescuer, emergency_admin)
    with boa.reverts("zeroaddr: bribe_manager"):
        IncentivesManager(crvusd, zero, bribe_proposer, token_rescuer, emergency_admin)
    ...
    with boa.reverts("zeroaddr: token_rescuer"):
        IncentivesManager(crvusd, bribe_manager, bribe_proposer, zero, emergency_admin)
    with boa.reverts("zeroaddr: emergency_admin"):
        IncentivesManager(crvusd, bribe_manager, bribe_proposer, token_rescuer, zero)
